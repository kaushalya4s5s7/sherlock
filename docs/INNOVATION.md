...
A second analyst agent, a graph neural net, or Jev choosing `BLOCK_CARD`. Two humans steering one run need an owner: the policy engine owns the action, the human owns approval, the agent owns the facts. A shared screen with no owner is the failure mode inside the multiplayer idea itself.
